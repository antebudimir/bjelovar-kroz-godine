L.Control.Window = L.Control.extend({
	...
	initialize: function (container, options) {
		...
		if (this.options.closeButton) {
			...
			this._closeButton.title = 'Zatvori prozor';
		}
	...
	show: function (position) {
		...
		if (this.options.position == 'topLeft') {
			...
			//this.showOn([left+width/2-thisWidth/2-margin,top+offset])  //default

			//my addition...
			if (window.innerWidth > 1023) {
				this.showOn([left + width / 2 - thisWidth / 2 - margin, offset]);
			} else {
			...
			this.showOn([
				left + width / 2 - thisWidth / 2 - margin,
				top + top + height / 2 - thisHeight / 2 - margin + offset,
			]);
		}
	...
	hide: function (e) {
		...
		this.fire('hide');
		this.remove(); // my addition: clean the DOM
	...
	setContentMaxHeight: function () {
		...
		if (this.options.title) {
			// margin += this._containerTitleBar.offsetHeight-36; default

			//my addition...
			if (window.innerWidth > 1023) {
				margin += this._containerTitleBar.offsetHeight - 236;
			} else {
				margin += this._containerTitleBar.offsetHeight - 36; //default
			} //...ends here
		}
